...
AS BEGIN
    ...
    FETCH NEXT FROM shopping_cart_cursor INTO @product_id, @price;
    ...
    FETCH FIRST FROM shopping_cart_cursor INTO @product_id, @price;

    WHILE @@FETCH_STATUS = 0
    BEGIN
        INSERT INTO order_details (order_id, product_id, price, status_id)
        VALUES (@order_id, @product_id, @price, IIF(
            @price = 0,
