...
BEGIN
    ...
    DECLARE @fast_activity_id INT = @slow_activity_id;

    WHILE @fast_activity_id IS NOT NULL AND
          @slow_activity_id <> @fast_activity_id
    BEGIN
        ...
        SET @fast_activity_id = dbo.get_make_up_presence_activity(
            dbo.get_make_up_presence_activity(@student_id, @fast_activity_id),
            @student_id
        );
    END
